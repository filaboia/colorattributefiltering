...
class ImageEvaluationFunctionTest(unittest.TestCase):
    ...
    def testZhangE(self):
        self.assertAlmostEqual(ZhangE(self.f, self.w), 1.6430741894285696)
    
    def testColorHarmonyWeighted(self):
        self.assertAlmostEqual(ColorHarmonyWeighted(self.f, self.w), -0.14, delta=0.01)
    
    def testColorHarmonySegmented(self):
        self.assertAlmostEqual(ColorHarmonySegmented(self.f, self.w), 0.11, delta=0.01)
